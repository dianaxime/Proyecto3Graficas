    ...
    def game_start(self):
        ...
        while running:
            ...
            if not paused:
                ...
                texto_de_salida = "Time: {0:02}:{1:02}".format(
                    minutos, segundos)
                texto = fuente.render(texto_de_salida, True, WHITE)
                segundos_totales = instante_de_partida - \
                    (numero_de_fotogramas // tasa_fotogramas)
                ...
                if segundos == 0:
                    self.game_over()
                texto_de_salida = "Time left: {0:02}:{1:02}".format(
                    minutos, segundos)
                texto = fuente.render(texto_de_salida, True, WHITE)
